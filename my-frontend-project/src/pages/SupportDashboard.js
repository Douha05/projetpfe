...
const STEPS = [
  { key:"ready_for_support",  label:"À faire",          next:null },
  { key:"in_progress",        label:"En cours",         next:"in_progress" },
  { key:"ready_for_customer", label:"Solution envoyée", next:"ready_for_customer" },
...
export default function SupportDashboard() {
  ...
  const stepClick = (step, idx) => {
    ...
    if (!["solved","closed","cancelled"].includes(selected.statut) && step.next && idx === cur + 1)
      changeStatut(selected._id, step.next);
  };
              ...
              {selected.fichiers?.length > 0 && (
                <div className="ticket-fichiers-section">
                  <p className="ticket-fichiers-title">
                    📎 Pièces jointes ({selected.fichiers.length})
                  </p>
                  <div className="ticket-fichiers-grid">
                    {selected.fichiers.map((f, i) => (
                      <div
                        key={i}
                        className="ticket-fichier-item"
                        onClick={() => setLightbox({ src: `http://localhost:3001${f.chemin}`, type: f.type })}
                      >
                        {f.type === "image" ? (
                          <img
                            src={`http://localhost:3001${f.chemin}`}
                            alt={f.nom}
                            className="fichier-thumb"
                          />
                        ) : (
              ...
              {selected.statut !== "cancelled" && (
                <div className="mini-kanban-section">
                  ...
                  <div className="mini-kanban-steps">
                    {STEPS.map((step, i) => {
                      const cur = IDX[selected.statut] ?? -1;
                      const done = i < cur, active = i === cur, esc = selected.statut === "escalated";
                      const canClick = !["solved","closed","cancelled"].includes(selected.statut) && step.next && i === cur + 1;
                      return (
