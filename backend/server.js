...
// ── Score compétence agents ──────────────────────────────
const { calculerScoresTousAgents } = require('./services/calculerScoreAgent');

// Calcul toutes les heures
cron.schedule('0 * * * *', async () => {
  await calculerScoresTousAgents();
});

// Calcul au démarrage (après 10 secondes)
setTimeout(async () => {
  await calculerScoresTousAgents();
}, 10000);